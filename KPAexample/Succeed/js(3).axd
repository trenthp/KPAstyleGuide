...
(function ($) {
	...
	function Tooltip(trigger, conf) {
		...
		function getPosition(e) {
			if (conf.position[0] == 'mouse') {
				var mouseX = e.clientX ? e.clientX : e.pageX;
				var mouseY = e.clientY ? e.clientY : e.pageY;
				var top = mouseY + conf.offset[0];
				var left = mouseX + conf.offset[1];
			} else {
				// get origin top/left position 
				var triggerOffset = trigger.offset();
				if (triggerOffset == null) {
					triggerOffset = {top:0, left:0};
				}
				var top = conf.relative ? trigger.position().top : triggerOffset.top;
				var left = conf.relative ? trigger.position().left : triggerOffset.left;
				pos = conf.position[0];
